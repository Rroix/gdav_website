...
const user = {
  ...
  role_label: "Dev",
  staff_access: true,
  capabilities: ["staff.access", "admin.access", "queue.view", "queue.claim", "queue.reassign", "queue.manage_state", "outreach.record", "tasks.assign", "tasks.manage_team", "notes.owner", "review.qa", "review.adjust_tier", "applications.review_judge", "appeals.review", "appeals.execute", "staff.manage", "staff.view", "staff.manage_standard_roles", "staff.manage_all", "staff.manage_nicknames", "requests.manage", "requests.schedule", "tracking.manage", "pps.manage_cycles", "pps.override", "operations.view", "audit.view", "audit.view_full", "config.manage_safe", "developer.access"],
};
const viewRoles = [
  ...
  ["admin", "Admin", ["staff.access", "admin.access", "queue.view", "applications.review_judge", "appeals.review", "appeals.execute", "staff.view"]],
  ["owner", "Owner", user.capabilities.filter((capability) => capability !== "developer.access")],
  ["dev", "Dev", user.capabilities],
];
const api = {
  version: legacyApi ? 0 : 8,
...
const payloads = {
  ...
  "/api/apply/options": { items: [{ application_type: "judge", label: "Reviewer application", description: "Review Geometry Dash levels and recommend rating tiers.", enabled: true, open: true, cooldown: { days: 5, active: false } }, { application_type: "mod", label: "Mod application", description: "Help moderate GD Avenue and support the community.", enabled: true, open: true, cooldown: { days: 5, active: false } }, { application_type: "appeal", label: "Punishment appeal", description: "Ask GD Avenue to review a ban, timeout, mute, or configured access restriction.", enabled: true, open: true, cooldown: { days: 30, active: false } }], applications_open: true, application_open_by_type: { judge: true, mod: true }, cooldown: { days: 5, active: false }, cooldowns: { judge: { days: 5, active: false }, mod: { days: 5, active: false } }, active_applications: [], active_application: null, guild_member: true },
  ...
  "/api/apply/form/appeal": { application: null, form: { application_type: "appeal", label: "Punishment appeal", description: "Ask GD Avenue to review a ban, timeout, mute, or configured access restriction.", estimated_minutes: 12, sections: ["Grounds", "Your account", "Evidence", "Requested outcome", "Confirmation"] }, punishment: { id: 4, type: "timeout", active: true, verified: true, reason: null, reason_known: false, reason_source: "unknown", reason_conflict: false, issued_ts: null, ends_ts: now + 7200, checked_ts: now - 60, lookup_status: "found", lookup_error: null }, manual_entry: { allowed: true, required: false, reason_missing: true, issued_date_missing: true, role_options: [{ id: "1159183038018768916", label: "Muted" }], value: {}, notice: "You may add missing context without replacing the Discord-observed record." }, eligibility: { eligible: true, verified: true, reason: "active_punishment", cooldown_until_ts: null }, questions: [
    { key: "primary_ground", label: "What is the main reason for your appeal?", type: "single_choice", required: true, section: "Grounds", options: ["Facts are incorrect", "Context or evidence was missed", "The rule was applied incorrectly", "The punishment is disproportionate", "I am asking for reconsideration", "The wrong account was punished", "Technical or automated error", "Other"] },
    { key: "chronology", label: "Describe what happened in chronological order", type: "long_text", required: true, section: "Your account", options: [], recommended_words: 180 },
    { key: "disputed_detail", label: "What part of the record or decision is inaccurate, and what should it say instead?", type: "long_text", required: false, section: "Your account", options: [], show_for: { primary_ground: ["Facts are incorrect", "Context or evidence was missed", "The rule was applied incorrectly", "The wrong account was punished", "Technical or automated error"] } },
    { key: "reconsideration", label: "What has changed, and how would you prevent the same situation from happening again?", type: "long_text", required: false, section: "Your account", options: [], show_for: { primary_ground: ["I am asking for reconsideration"] } },
    { key: "evidence_links", label: "Evidence links and what each one shows", type: "long_text", required: false, section: "Evidence", options: [] },
    { key: "requested_outcome", label: "What outcome are you requesting?", type: "single_choice", required: true, section: "Requested outcome", options: ["Remove the punishment", "Reduce the punishment", "Correct the record", "Review the decision", "Other"] },
    { key: "confirmation", label: "Confirm the appeal is accurate and submitted in good faith", type: "single_choice", required: true, section: "Confirmation", options: ["I confirm"] },
  ] },
  ...
  "/api/staff/appeals": { items: [{ id: 22, appellant_id: "887766554433221100", appellant: { id: "887766554433221100", display_name: "Outside Applicant", avatar_url: "" }, status: "under_review", claimed_by: user.id, submitted_ts: now - 7200, punishment_type: "ban", punishment_active: true, reason: "Repeated harassment after a moderator warning", reason_source: "discord_ban", reason_conflict: true, issued_ts: now - 86400 * 12, issued_by_id: "554433221100998877", issued_by_identity: { id: "554433221100998877", display_name: "Moderator" }, audit_log_entry_id: "998877665544332211", lookup_status: "found", source_detail: { live_reason: "Repeated harassment after a moderator warning", audit_reason: "Harassment after warning", sapphire_note: "Sapphire private case data was not queried." }, answers: { primary_ground: "Context or evidence was missed", chronology: "The applicant supplied a chronological account and asked staff to verify the attached context.", disputed_detail: "The second exchange happened before the warning.", evidence_links: "https://example.com/evidence", requested_outcome: "Remove the punishment", confirmation: "I confirm" }, assessments: [{ reviewer_id: "101010101010101010", reviewer: { id: "101010101010101010", display_name: "Reviewer One" }, recommendation: "removed", rationale: "The supplied timestamps materially change the chronology.", findings: { factual_accuracy: "One event is ordered incorrectly.", proportionality: "Review after correcting the timeline." }, recused: 0 }], messages: [{ id: 1, author_type: "applicant", body: "I added a link showing the missing context.", created_ts: now - 1800 }], decision_ready: { ready: false, eligible_assessments: 1, minimum: 2, issuer_excluded: true }, staff_unread_count: 1 }] },
  ...
  "/api/staff/operations": { service: { state: "online", detail: "All core services ready" }, runtime: { ready: true, responsive: true }, database: { connected: true }, outbox: { pending: 1 }, request_wave: { state: "open" }, providers: {}, creator_points_providers: { boomlings: { status: "healthy", attempts: 8, success_rate: 0.875, cp_success_rate: 0.75, identity_success_rate: 1, median_latency_ms: 138.4, parse_failures: 0, last_success: now - 420, methods: [{ method: "level", attempts: 4, cp_successes: 0, parse_failures: 0 }, { method: "direct_profile", attempts: 4, cp_successes: 4, parse_failures: 0 }] }, gdbrowser: { status: "degraded", attempts: 8, success_rate: 0.5, cp_success_rate: 0.375, identity_success_rate: 0.75, median_latency_ms: 215.2, parse_failures: 1, last_success: now - 1200, methods: [{ method: "html_level", attempts: 4, cp_successes: 0, parse_failures: 0 }, { method: "html_profile", attempts: 3, cp_successes: 3, parse_failures: 0 }, { method: "api_profile", attempts: 1, cp_successes: 0, parse_failures: 1 }] } }, background_workers: { "priority.maintenance": "running", "priority.creator_points": "running" }, pps_worker: "running", public_cache: { state: "available" }, incidents: [] },
  ...
  "/api/staff/configuration": { configuration: { claim_stale_hours: 48, applications_open: true, application_open_by_type: { judge: true, mod: true }, appeals_open: true } },
};
