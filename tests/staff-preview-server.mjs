...
const user = {
  ...
  role: "owner",
  staff_access: true,
  capabilities: ["staff.access", "queue.view", "queue.claim", "queue.reassign", "queue.manage_state", "outreach.record", "tasks.assign", "tasks.manage_team", "notes.owner", "review.qa", "review.adjust_tier", "applications.review_judge", "staff.manage", "pps.manage_cycles", "pps.override", "operations.view", "audit.view", "config.manage_safe"],
};
const queue = [
...
const payloads = {
  ...
  "/api/staff/team": { review_progress: { done: 34, total: 48 }, queue: { queued: 17 }, claims: { active: 6, stale: 1 }, outreach_week: { attempts: 8, submissions: 4 }, pending_applications: 3, workload: [{ user_id: user.id, active_claims: 2 }, { user_id: "785212232786640966", active_claims: 4 }] },
  "/api/staff/statistics": { scope: "team", reviewers: [{ reviewed_by: "785212232786640966", reviews: 28, median_turnaround: 14400 }, { reviewed_by: user.id, reviews: 24, median_turnaround: 10800 }], tiers: { rate: 9, feature: 12, epic: 18, legendary: 8, mythic: 5 }, results: { sent: 41, rejected: 9, other: 2 }, outreach: { attempts: 18, submissions: 8, followups: 4, routes: [{ route_type: "direct", attempts: 10, submissions: 5 }, { route_type: "network", attempts: 8, submissions: 3 }] }, tasks_completed: 16, active_weeks: 12, stale_claims: 1, queue: { queued: 17, in_cycle: 4, awaiting_outcome: 6, rated: 9 }, waiting_distribution: { 0: 8, 1: 5, 2: 4, "3": 2, "4+": 2 }, cp_distribution: { 0: 6, 1: 5, 2: 3, 3: 2, "4+": 2, unknown: 3 } },
  "/api/staff/qa": { items: [{ request_message_id: 9001, level_id: "101935961", result: "sent", send_type: "mythic", reviewed_by: "785212232786640966", qa_status: "unreviewed" }] },
  "/api/staff/applications": { items: [{ id: 14, applicant_id: "998877665544332211", submitted_ts: now - 86400, status: "under_review", answers: { motivation: "I want to help creators receive clear, consistent feedback.", experience: "Two years of level reviewing and community moderation.", availability: "Four evenings each week." }, timeline: [{ event: "submitted", created_ts: now - 86400 }, { event: "claim", created_ts: now - 7200 }], internal_notes: [{ body: "Strong examples; discuss availability in interview.", created_ts: now - 3600 }] }] },
