...
async function search() {
  ...
  try {
    ...
    results.innerHTML = data.levels.length ? data.levels.map((item) => `<a class="public-level-row" href="/level/${esc(item.level_id)}"><span class="public-level-identity"><strong>${esc(item.level_name)}</strong><small>${esc(item.uploader_name || "Unknown creator")} · <code>${esc(item.level_id)}</code></small></span><span class="public-tier ${esc(item.recommendation_type)}">${tierArt(item.recommendation_type)}<span>${esc(label(item.recommendation_type))}</span></span><span class="public-level-state"><small>Status</small>${esc(label(item.public_outcome_state === "rated" ? "rated" : item.public_outreach_state))}</span><span class="priority-band"><small>Priority</small>${item.public_priority_band ? esc(label(item.public_priority_band)) : "Status available"}</span><span class="public-row-arrow" aria-hidden="true">→</span></a>`).join("") : '<div class="empty-state"><strong>No public recommendation found</strong><span>Only levels recommended by GD Avenue appear here.</span></div>';
  } catch (error) {
